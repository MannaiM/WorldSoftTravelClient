...
@LocalBean
@Stateless
public class MvmentPrixVolRetourService {
	@PersistenceContext
	EntityManager em;
	...
	public List<MvmentPrixVolResultat> selectPrixVolRetour(MvmentPrixVolModel mvmentPrixVolModel) {
		...
		if (mvmentPrixVolModel.getAirDepM() != null && mvmentPrixVolModel.getAirArrM() != null) {
			...
			predicates.add(criteriaBuilder.equal(r.get("dep"), mvmentPrixVolModel.getAirArrM()));

			predicates.add(criteriaBuilder.equal(r.get("arr"), mvmentPrixVolModel.getAirDepM()));
		...
		if (mvmentPrixVolModel.getDateRetM() != null) {
			try {
				mvmentPrixVolModel.setDateRetM(mvmentPrixVolModel.getDateRetM().substring(0, 10));
				SimpleDateFormat FormattedDATE = new SimpleDateFormat("yyyy-MM-dd");

				predicates.add(criteriaBuilder.equal(r.<Date> get("dateRes"),
						FormattedDATE.parse(mvmentPrixVolModel.getDateRetM())));
